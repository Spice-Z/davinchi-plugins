...
describe("fcpxml cutter", () => {
  ...
  it("processes fcpxml with mock analyzer", async () => {
    ...
    const clips = xpathElements(".//*[local-name()='asset-clip']", root);
    expect(clips.length).toBe(3);
    expect(clips[0]!.getAttribute("offset")).toBe("0/1s");
    expect(clips[1]!.getAttribute("offset")).toBe("2/1s");
    expect(clips[0]!.getAttribute("duration")).toBe("1/1s");
    expect(clips[1]!.getAttribute("duration")).toBe("1/1s");
  });
});
